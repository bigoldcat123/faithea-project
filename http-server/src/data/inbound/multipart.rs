...
impl TryFrom<Part> for MultiPartFile {
    ...
    fn try_from(value: Part) -> Result<Self, Self::Error> {
        if let Part::File { file_name, data, mime_type } = value {
            Ok(Self { file_name, data, mime_type })
        }else {
...
fn handler(b: &[u8], boundary: &[u8],data:&mut HashMap<String,Part>) {
    ...
    while r < b.len() {
        ...
        if file_name.is_some() || mime_type.is_some() {
            data.insert(name.to_string(), Part::File { file_name, data: Bytes::copy_from_slice(&b[l..r]), mime_type });
        }else {
            data.insert(name.to_string(), Part::Lit(String::from_utf8_lossy(&b[l..r]).to_string()));
        }
...
fn process_multipart_header<'a>(h:&'a [u8],name:&mut &'a str,file_name:&mut Option<String>,mime_type:&mut Option<String>)  {
    if let Ok(h) = str::from_utf8(h) {
